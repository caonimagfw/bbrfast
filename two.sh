...
pa2=$2
tls="TLS13_CHACHA20_POLY1305_SHA256"
if [[ ${3} == "aes" ]]; then
    tls="TLS13_AES_128_GCM_SHA256"
fi
...
d4(){
    echo "tls:${tls}"
    docker run --name trojan-gnu-r -itd --restart=unless-stopped --network host trojan-gnu /usr/local/bin/tr-gnu 0.0.0.0:10111 ${pa2} -t 20 -d 8.8.4.4 -c /etc/trojan/server.cert.pem -k /etc/trojan/server.key.pem -s ${tls}
}

d6(){
    echo "tls:${tls}"
    docker run --name trojan-gnu-r -itd --restart=unless-stopped --network host trojan-gnu /usr/local/bin/tr-gnu 0.0.0.0:10111 ${pa2} -t 20 -c /etc/trojan/server.cert.pem -k /etc/trojan/server.key.pem -s ${tls}
}
ds(){
	sysctl net.ipv4.tcp_available_congestion_control
	sysctl net.ipv4.tcp_congestion_control
